...
func createDynamicFrontendFile(tp *model.ThePackage, et *model.TheEntity) {
	...
	// create router modules
	{
		...
		outputFile := fmt.Sprintf("../../../../%s/frontend/src/router/modules/%s.js", tp.PackagePath, strings.ToLower(et.Name))
		basic(tp, templateFile, outputFile, et, 0664)
	}

	{
		// create folder pages
		f := fmt.Sprintf("../../../../%s/frontend/src/pages/%s", tp.PackagePath, strings.ToLower(et.Name))
		os.Mkdir(f, 0777)

		// create file table under folder
		{
			templateFile := fmt.Sprintf("../templates/frontend/src/pages/folder/list._vue")
			outputFile := fmt.Sprintf("../../../../%s/frontend/src/pages/%s/list.vue", tp.PackagePath, strings.ToLower(et.Name))
			basic(tp, templateFile, outputFile, et, 0664)
		}
		...
		// create file input under folder
		{
			...
			outputFile := fmt.Sprintf("../../../../%s/frontend/src/pages/%s/input.vue", tp.PackagePath, strings.ToLower(et.Name))
			basic(tp, templateFile, outputFile, et, 0664)
		}
